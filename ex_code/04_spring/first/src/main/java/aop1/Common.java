...
public class Common {
	...
	public void a() {
		System.out.println
		("===> a 메소드는 핵심관심의 사전 호출 : " + new Date(starttime));
	}
	
	public void b() {
		long endtime = System.currentTimeMillis(); //1/1000초
		System.out.println
		("===> b 메소드는 핵심관심의 이후 호출 (수행시간) : " + (endtime-starttime));

	}

	public void c(ProceedingJoinPoint joinpoint) {// c 핵심클래스 정보객체
		try {
		System.out.println
