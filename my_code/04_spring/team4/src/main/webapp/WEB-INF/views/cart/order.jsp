...
<tr><td>test</td><td>${cart.title}</td><td>${cart.price}</td><td>${cart.count}</td><td>  ${cart.price * cart.count}  </td><td><input type=button value="삭제"></td></tr>
	...
	<div id= "cart_total">
		<table id="cart_total_table">
			...
			<tr><td>${totalcount.get(0).totalcount }</td><td>${total.get(0).sumproduct }</td><td>3000</td><td>=</td><td>${total.get(0).sumproduct + 3000}</td></tr>
		...
		<input type="button" value="돌아가기" onclick="history.back(-1);"><input id='order' name='order' type="button" value="주문하기">
