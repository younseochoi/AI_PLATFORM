...
$(document).ready(function(){
	...
	myimage.src = "http://localhost:8082/naverai/${param.image}";
		...
		for(let i=0;i<json.predictions.length;i++){ //이미지에 있는 사람수 만큼 반복
			for(let j=0;j<bodyinforms.length;j++){ //신체 수만큼 반복
				if(json.predictions[i][j]!=null){
				var x = json.predictions[i][j].x * myimage.width;
				var y = json.predictions[i][j].y * myimage.height;
				mycontext.fillStyle=colors[j];
				mycontext.fillText(bodyinforms[j]+"("+json.predictions[i][j].score+')',x,y);
				mycontext.fillRect(x,y,5,5); 
				}
			//양손목 연결선그리기
			/* 	if(bodyinforms[j].indexOf("왼손목")>=0){
					var leftx = json.predictions[i][j].x * myimage.width;
					var lefty = json.predictions[i][j].y * myimage.height;
				}
				if(bodyinforms[j].indexOf("오손목")>=0){
					var rightx = json.predictions[i][j].x * myimage.width;
					var righty = json.predictions[i][j].y * myimage.height;
				}
				mycontext.beginPath();
...
<canvas id="mycanvas" style="border:2px solid orange;" width="500px" height="500px"></canvas>
